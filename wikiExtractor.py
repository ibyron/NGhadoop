...
def clean(text):
    ...
    # Byron 05/08/2013 
    text = re.sub(r'{{quote\|([^{]+)}}', r'\1', text)							# retain quotes
    text = re.sub(r'{{convert\|([^\|]+)\|([^\|]+)(\|[^ ]+)+}}', r'\1 \2', text)	# retain convert (unit/measures)
    text = re.sub(r'\({{IPAc.*or {{IPA.*}}\)', '', text)					# discard IPA
    text = re.sub(r'\[\[wikt:[^ ]+\|([^ ]+)\]\]', r'\1', text)				# retain text in wikt
